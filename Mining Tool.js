...
function NavLink({ children, active, onClick }) {
    return (
        <button
            onClick={onClick}
            className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all shrink-0 ${active
...
// SliderInput with fully unrestricted, manual, unbounded typing box
                ...
                <div className="flex items-center space-x-1.5 shrink-0">
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            onChange(isNaN(val) ? e.target.value : val);
                        }}
                        className="w-24 bg-slate-900 text-amber-400 font-mono font-bold text-xs text-right px-2 py-1 rounded border border-slate-800 focus:outline-none focus:border-amber-500"
                    />
...
function calculateSlopeFoS(state, isSaturated) {
    const h = Math.max(1, parseFloat(state.height) || 0);
    const angle = Math.max(5, Math.min(89, parseFloat(state.angle) || 0));
    const beta = (angle * Math.PI) / 180;
    const cohesion = Math.max(0, parseFloat(state.cohesion) || 0);
    const phi = (Math.max(0, Math.min(89, parseFloat(state.frictionAngle) || 0)) * Math.PI) / 180;
    const gamma = Math.max(1, parseFloat(state.unitWeight) || 0);
    const waterDepth = Math.max(0, parseFloat(state.waterTableDepth) || 0);

    const tanBeta = Math.tan(beta);
    const crestX = 50;
    const toeX = crestX + h / tanBeta;
    const toeY = 50;

    // Slip circle center positioning for slope toe failure
    const centerX = toeX + 0.1 * h;
    const centerY = toeY + 1.35 * h;
    const radius = Math.sqrt(Math.pow(centerX - toeX, 2) + Math.pow(centerY - toeY, 2));

    // Entry point behind crest on upper bench surface
    const dy = centerY - (toeY + h);
    let xStart = crestX;
    if (radius * radius > dy * dy) {
        xStart = centerX - Math.sqrt(radius * radius - dy * dy);
    }
    if (isNaN(xStart) || xStart >= toeX) {
        xStart = crestX - 10;
    }

    const numSlices = 20;
    const sliceWidth = (toeX - xStart) / numSlices;

    let sumResisting = 0;
    let sumDriving = 0;

    for (let k = 0; k < numSlices; k++) {
        const sliceMidX = xStart + (k + 0.5) * sliceWidth;
        let surfY = toeY + h;
        if (sliceMidX > toeX) surfY = toeY;
        else if (sliceMidX >= crestX) surfY = toeY + (toeX - sliceMidX) * tanBeta;

        const dxCircle = sliceMidX - centerX;
        const rSq = radius * radius;
        const dxSq = dxCircle * dxCircle;
        if (dxSq >= rSq) continue;

        const slipY = centerY - Math.sqrt(rSq - dxSq);
        const sliceH = surfY - slipY;
        if (sliceH <= 0) continue;

        const weight = sliceWidth * sliceH * gamma;
        
        // Base inclination angle alpha for slice (positive downwards towards toe)
        const sinAlpha = Math.max(0.01, Math.min(0.99, (centerX - sliceMidX) / radius));
        const alphaRad = Math.asin(sinAlpha);
        const cosAlpha = Math.cos(alphaRad);
        const dl = sliceWidth / Math.max(0.1, cosAlpha);

        let u = 0;
        if (isSaturated) {
            const phreaticY = (toeY + h) - waterDepth;
            if (slipY < phreaticY) {
                // Pore water pressure: depth of water column above slip arc * 9.81 kN/m³
                u = Math.max(0, (phreaticY - slipY) * 9.81);
            }
        }

        const N_eff = Math.max(0, weight * cosAlpha - u * dl);
        const resisting = cohesion * dl + N_eff * Math.tan(phi);
        const driving = weight * sinAlpha;

        sumResisting += resisting;
        sumDriving += driving;
    }

    if (sumDriving <= 0.001) return 9.99;
    const calculated = sumResisting / sumDriving;
    return isNaN(calculated) || calculated <= 0 ? 0.01 : Math.min(9.99, calculated);
}
                                ...
                                <div className="space-y-2">
                                    {Object.entries(ROLES).map(([key, roleObj]) => (
                                        <button
                                            ...
                                            className={`w-full text-left p-3 rounded-lg border text-xs flex justify-between items-center transition ${userProfile.role === key
...
function StrippingRatioPage({ state, setState }) {
    const plannedSR = parseFloat(state.plannedSR) || 0;
    const sellingPrice = parseFloat(state.sellingPrice) || 0;
    const processingCost = parseFloat(state.processingCost) || 0;
    const miningCost = parseFloat(state.miningCost) || 0;
    const wasteCost = parseFloat(state.wasteCost) || 0;

    const profitMarginBeforeStripping = sellingPrice - processingCost - miningCost;
    const breakEvenSR = wasteCost > 0 ? (profitMarginBeforeStripping / wasteCost) : 0;

    const netMargin = profitMarginBeforeStripping - (plannedSR * wasteCost);
        ...
        >
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-5 flex flex-col space-y-6">
                    ...
                    <div className={`border rounded-xl p-6 shadow-lg text-center ${isProfitable
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 space-y-3 shadow-lg shadow-slate-950/50">
                        ...
                        <div className="flex justify-between text-xs py-1 border-b border-slate-900">
                            ...
                            <span className="font-mono text-emerald-400">${sellingPrice.toFixed(2)}/t</span>
                        </div>
                        <div className="flex justify-between text-xs py-1 border-b border-slate-900">
                            <span className="text-slate-400">Direct Mining & Processing Costs</span>
                            <span className="font-mono text-rose-400">${(processingCost + miningCost).toFixed(2)}/t</span>
                        </div>
                        <div className="flex justify-between text-xs py-1 border-b border-slate-900">
                        ...
                        <div className="flex justify-between text-xs py-1">
                            ...
                            <span className="font-mono text-rose-400">-${(plannedSR * wasteCost).toFixed(2)}/t</span>
                        </div>
                    </div>
                </div>
            </div>
...
function PitSensitivityPage() {
    const [scenarios, setScenarios] = useState(() => {
        try {
            const saved = localStorage.getItem('mining_pit_scenarios');
    ...
    useEffect(() => {
        try {
            localStorage.setItem('mining_pit_scenarios', JSON.stringify(scenarios));
        } catch (e) {
            console.warn("Storage write failed:", e);
        }
    }, [scenarios]);
    ...
    const activePrice = BASE_PRICE * (1 + (parseFloat(priceAdjustment) || 0) / 100);
    const activeOreCost = BASE_ORE_COST * (1 + (parseFloat(costAdjustment) || 0) / 100);
    const activeWasteCost = BASE_WASTE_COST * (1 + (parseFloat(costAdjustment) || 0) / 100);

    const computedData = scenarios.map(sc => {
        const tonnesOre = parseFloat(sc.recoverableVolume) || 0;
        const wasteVolume = tonnesOre * (parseFloat(sc.strippingRatio) || 0);
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 space-y-4 font-mono shadow-lg shadow-slate-950/50">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-slate-900 p-3 rounded-lg border border-slate-800">
                                ...
                                <span className="block text-lg font-mono font-bold text-slate-100">{computedData.reduce((acc, curr) => acc + (parseFloat(curr.recoverableVolume) || 0), 0).toLocaleString()} t</span>
                            </div>
                            <div className="bg-slate-900 p-3 rounded-lg border border-slate-800">
...
function FleetSizingPage({ state, setState }) {
    const bucketCap = parseFloat(state.bucketCap) || 0;
    const bucketCycle = parseFloat(state.bucketCycle) || 0;
    const truckCap = parseFloat(state.truckCap) || 0;
    const haulDist = parseFloat(state.haulDist) || 0;
    const haulSpeed = parseFloat(state.haulSpeed) || 0;
    const dumpTime = parseFloat(state.dumpTime) || 0;
    const numTrucks = parseFloat(state.numTrucks) || 0;
    const numLoaders = parseFloat(state.numLoaders) || 0;

    const speedMPS = (haulSpeed * 1000) / 3600;
    const travelTimeOneWay = speedMPS > 0 ? (haulDist / speedMPS) / 60 : 0;
    const travelTimeRoundTrip = travelTimeOneWay * 2;

    const passesNeeded = bucketCap > 0 ? Math.ceil(truckCap / bucketCap) : 0;
    const calculatedLoadTime = (passesNeeded * bucketCycle) / 60;

    const totalTruckCycleTime = calculatedLoadTime + travelTimeRoundTrip + dumpTime;

    const matchFactor = (totalTruckCycleTime > 0 && numLoaders > 0)
        ? (numTrucks * calculatedLoadTime) / (totalTruckCycleTime * numLoaders)
        : 0;

    const theoreticalRecommendedTrucks = calculatedLoadTime > 0 ? (totalTruckCycleTime * numLoaders) / calculatedLoadTime : 0;

    const operationalEfficiency = 0.83;
    const loaderProdTonesHr = (calculatedLoadTime > 0 && totalTruckCycleTime > 0 && numTrucks > 0)
        ? (truckCap / (totalTruckCycleTime / numTrucks)) * 60 * operationalEfficiency
        : 0;
        ...
        >
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-5 space-y-6">
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 text-center shadow-lg shadow-slate-950/50">
                        <h3 className="text-sm font-semibold tracking-wide uppercase text-slate-400 font-mono">Calculated Match Factor</h3>
...
function CapexOpexPage({ state, setState }) {
    ...
    const laborRate = parseFloat(state.laborRate) || 0;
    const maintenanceFactor = parseFloat(state.maintenanceFactor) || 0;
    const prodVolume = parseFloat(state.prodVolume) || 0;
    ...
    state.capexList.forEach(asset => {
        ...
        const assetUnitCost = parseFloat(asset.unitCost) || 0;
        const assetLife = parseFloat(asset.life) || 0;

        const totalCost = assetUnitCost * assetQty;
        totalCapexValue += totalCost;
        if (i > 0 && assetLife > 0) {
            const pmfFactor = (i * Math.pow(1 + i, assetLife)) / (Math.pow(1 + i, assetLife) - 1);
            totalAnnualizedCapex += totalCost * pmfFactor;
        } else if (assetLife > 0) {
            totalAnnualizedCapex += totalCost / assetLife;
        }
    });

    const estimatedFuelLitres = prodVolume * 0.45;
    const estimatedManHours = (prodVolume / 1000) * 1.5;
    const annualFuelCost = estimatedFuelLitres * fuelPrice;
    const annualLaborCost = estimatedManHours * laborRate;
    const annualMaintenanceCost = totalCapexValue * (maintenanceFactor / 100);
    ...
    const unitCostPerTonne = prodVolume > 0 ? (totalAnnualCost / prodVolume) : 0;
    ...
    const getUnitCostWithShift = (param, shiftPercent) => {
        ...
        let shiftedProdVol = param === 'prod' ? prodVolume * shift : prodVolume;
...
function SlopeStabilityPage({ state, setState }) {
    ...
    const dryFoS = calculateSlopeFoS(state, false);
    const wetFoS = calculateSlopeFoS(state, true);

    const angle = Math.max(1, Math.min(89, parseFloat(state.angle) || 0));
    const height = Math.max(1, parseFloat(state.height) || 0);
    const betaRad = (angle * Math.PI) / 180;
    const safeTanBeta = Math.max(0.01, Math.tan(betaRad));
    ...
    useEffect(() => {
        ...
        const scale = Math.min(3, 180 / Math.max(1, height));
        const startX = 30;
        const startY = 240;
        ...
        const pxCrestY = startY - height * scale;
        const pxToeX = pxCrestX + (height / safeTanBeta) * scale;
        const pxToeY = startY;
        ...
        const waterDepth = parseFloat(state.waterTableDepth) || 0;
        const pxWaterY = pxCrestY + waterDepth * scale;
        if (pxWaterY < pxToeY && pxWaterY > pxCrestY) {
            ...
            const waterContactX = pxToeX - (waterDepth / safeTanBeta) * scale;
        ...
        const pxCenterX = pxToeX + 0.1 * height * scale;
        const pxCenterY = pxToeY - 1.35 * height * scale;
        const pxRadius = Math.sqrt(Math.pow(pxCenterX - pxToeX, 2) + Math.pow(pxCenterY - pxToeY, 2));

        ctx.beginPath();
        const startAngle = Math.atan2((pxToeY - height * scale) - pxCenterY, pxCrestX - pxCenterX);
        const endAngle = Math.atan2(pxToeY - pxCenterY, pxToeX - pxCenterX);
        ctx.arc(pxCenterX, pxCenterY, Math.max(1, pxRadius), startAngle, endAngle);
        ...
        ctx.fillText(`Crest: h=${height}m, β=${angle}°`, pxCrestX + 10, pxCrestY - 10);
        ctx.fillText('Potential Slip Circle Arc', pxCenterX - 40, Math.max(15, pxCenterY - pxRadius - 10));

    }, [height, angle, state.waterTableDepth, state.cohesion, state.frictionAngle, state.unitWeight, safeTanBeta]);

    return (
        <PageLayout title="FoS Geotechnical Stability Solver" description="Perform dynamic Limit Equilibrium slope analyses. Compares factor of safety thresholds under dry and saturated loading to prevent catastrophic bench wall sliding.">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <div className="lg:col-span-7 space-y-6">
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 space-y-6">
                        ...
                        <SliderInput label="Water Table Depth below Crest (m)" value={state.waterTableDepth} onChange={(val) => setState({ ...state, waterTableDepth: val })} min={0} max={height} step={1} unit="m" />
                    </div>
                </div>
...
function GradeBlendingPage({ state, setState }) {
    ...
    const targetAsh = parseFloat(state.targetAsh) || 0;
    const targetMoisture = parseFloat(state.targetMoisture) || 0;
    const targetCV = parseFloat(state.targetCV) || 0;
    ...
    useEffect(() => {
        ...
        const evaluateError = (ratios) => {
            ...
            ratios.forEach((ratio, index) => {
                mixedAsh += ratio * (parseFloat(state.sources[index]?.ash) || 0);
                mixedMoist += ratio * (parseFloat(state.sources[index]?.moisture) || 0);
                mixedCV += ratio * (parseFloat(state.sources[index]?.cv) || 0);
            });
            return (wAsh * Math.pow(mixedAsh - targetAsh, 2) + wMoisture * Math.pow(mixedMoist - targetMoisture, 2) + wCV * Math.pow(mixedCV - targetCV, 2));
        };
        ...
        bestX.forEach((ratio, index) => {
            finalAsh += ratio * (parseFloat(state.sources[index]?.ash) || 0);
            finalMoist += ratio * (parseFloat(state.sources[index]?.moisture) || 0);
            finalCV += ratio * (parseFloat(state.sources[index]?.cv) || 0);
        });

        setResultingQualities({ ash: finalAsh, moisture: finalMoist, cv: finalCV });
        setIsInfeasible(Math.abs(finalAsh - targetAsh) > 2.0 || Math.abs(finalCV - targetCV) > 250);
    }, [state.sources, targetAsh, targetMoisture, targetCV]);
                        ...
                        {resultingQualities && (
                            <div className="space-y-3 font-mono">
                                ...
                                <div className="bg-slate-900 p-3 rounded-lg border border-slate-800 space-y-2 text-xs">
                                    <div className="flex justify-between"><span className="text-slate-400">Ash Composition:</span><span className="text-slate-200 font-bold font-mono">{resultingQualities.ash.toFixed(2)}% (vs {targetAsh}%)</span></div>
                                    <div className="flex justify-between"><span className="text-slate-400">Moisture Profile:</span><span className="text-slate-200 font-bold font-mono">{resultingQualities.moisture.toFixed(2)}% (vs {targetMoisture}%)</span></div>
                                    <div className="flex justify-between"><span className="text-slate-400">Calorific Energy:</span><span className="text-emerald-400 font-bold font-mono">{resultingQualities.cv.toFixed(0)} kcal/kg (vs {targetCV})</span></div>
                                </div>
                            </div>
                        )}
                        <div className="space-y-3 font-mono">
...
function OreVisualizationPage() {
    ...
    const numericCutoffGrade = parseFloat(cutoffGrade) || 0;
    const numericDepositVariance = parseFloat(depositVariance) || 0;
    ...
    const blockModel = useMemo(() => {
        ...
        for (let y = 0; y < gridSizeY; y++) {
            for (let z = 0; z < gridSizeZ; z++) {
                for (let x = 0; x < gridSizeX; x++) {
                    ...
                    const varianceFactor = numericDepositVariance / 100;
        ...
        return blocks;
    }, [gridSizeX, gridSizeY, gridSizeZ, numericDepositVariance, oreDensity]);
    ...
    }, [blockModel, selectedProp, numericCutoffGrade, oreDensity]);
    ...
    const getBlockColorRGB = (grade, rawValues = false) => {
        if (grade < numericCutoffGrade) {
            return rawValues ? [71, 85, 105] : `rgba(71, 85, 105, 0.15)`;
        } else {
            const ratio = Math.min(1.0, (grade - numericCutoffGrade) / 2.0);
    ...
    useEffect(() => {
        ...
        const projectedBlocks = blockModel.map(b => {
            ...
            const isOre = gradeVal >= numericCutoffGrade;
    ...
    }, [blockModel, viewMode, pitch, yaw, zoom, selectedProp, numericCutoffGrade, gridSizeX, gridSizeY, gridSizeZ, showWaste]);
                        ...
                        <div className="space-y-3 font-mono text-xs">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <button
                                    ...
                                    className={`py-2 px-3 rounded-lg text-xs font-bold transition border font-mono ${selectedProp === 'copper'
                                ...
                                <button
                                    ...
                                    className={`py-2 px-3 rounded-lg text-xs font-bold transition border font-mono ${selectedProp === 'gold'
                                            ? 'bg-amber-500/10 border-amber-500 text-amber-300 shadow'
                                            : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-700'
                                        }`}
                                >
                        ...
                        <div className="space-y-3 font-mono text-xs">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <button
                                    ...
                                    className={`py-2 px-3 rounded-lg text-xs font-bold transition border font-mono ${viewMode === '3D'
                                            ? 'bg-amber-500/10 border-amber-500 text-amber-300 shadow'
                                            : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-700'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`py-2 px-3 rounded-lg text-xs font-bold transition border font-mono ${viewMode === '2D'
                                            ? 'bg-amber-500/10 border-amber-500 text-amber-300 shadow'
                                            : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-700'
                                        }`}
                                >
                        ...
                        <div className="border-t border-slate-900 pt-4 space-y-3 font-mono text-xs">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1 rounded text-[10px] font-bold border transition ${showWaste ? 'bg-amber-500/15 border-amber-500 text-amber-300' : 'bg-slate-900 border-slate-800 text-slate-500'
                                        }`}
                                >
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1 rounded text-[10px] font-bold border transition ${autoRotate ? 'bg-amber-500/15 border-amber-500 text-amber-300' : 'bg-slate-900 border-slate-800 text-slate-500'
                                        }`}
                                >
                            ...
                            <div className="relative overflow-x-auto p-4 bg-slate-900 rounded-xl border border-slate-800 flex justify-center">
                                ...
                                >
                                    {blockModel.filter(b => b.y === Math.floor(gridSizeY / 2)).map((block) => {
                                        ...
                                        const isOre = grade >= numericCutoffGrade;
                                        return (
                                            <div
                                                key={block.id}
                                                ...
                                                className={`relative aspect-square rounded cursor-crosshair transition-all border duration-100 flex items-center justify-center ${isOre ? 'border-amber-500/20 hover:scale-110 hover:shadow-lg hover:shadow-amber-500/50 z-10' : 'border-slate-800 hover:scale-105 z-0'
                                                    }`}
                                            >
                            ...
                            <div className="mt-4 flex flex-wrap gap-4 items-center justify-between text-xs text-slate-400 font-mono">
                                <div className="flex gap-4">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>Waste (&lt; {numericCutoffGrade.toFixed(1)})</span>
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                        <div className="h-3.5 w-3.5 rounded bg-amber-500 border border-amber-600" />
                                        <span>Ore (&gt;= {numericCutoffGrade.toFixed(1)})</span>
                                    </div>
                                </div>
...
function ReportGeneratorPage({ userProfile, strippingRatio, fleet, capexOpex, slope, blending }) {
    ...
    const plannedSR = parseFloat(strippingRatio.plannedSR) || 0;
    const sellingPrice = parseFloat(strippingRatio.sellingPrice) || 0;
    const processingCost = parseFloat(strippingRatio.processingCost) || 0;
    const miningCost = parseFloat(strippingRatio.miningCost) || 0;
    const wasteCost = parseFloat(strippingRatio.wasteCost) || 0;

    const operatingProfitMargin = sellingPrice - processingCost - miningCost;
    const breakEvenLimit = wasteCost > 0 ? (operatingProfitMargin / wasteCost) : 0;

    const isStrippingProfitable = breakEvenLimit >= plannedSR;

    const speedMPS = ((parseFloat(fleet.haulSpeed) || 0) * 1000) / 3600;
    const travelTimeMin = speedMPS > 0 ? ((parseFloat(fleet.haulDist) || 0) / speedMPS) / 60 * 2 : 0;

    const fBucketCap = parseFloat(fleet.bucketCap) || 0;
    const fTruckCap = parseFloat(fleet.truckCap) || 0;
    const fBucketCycle = parseFloat(fleet.bucketCycle) || 0;
    const fNumTrucks = parseFloat(fleet.numTrucks) || 0;
    const fNumLoaders = parseFloat(fleet.numLoaders) || 0;
    const fDumpTime = parseFloat(fleet.dumpTime) || 0;

    const passes = fBucketCap > 0 ? Math.ceil(fTruckCap / fBucketCap) : 0;
    const loadingTimeMin = (passes * fBucketCycle) / 60;
    const totalCycleMin = loadingTimeMin + travelTimeMin + fDumpTime;
    const fleetMatchFactor = totalCycleMin > 0 ? (fNumTrucks * loadingTimeMin) / (totalCycleMin * fNumLoaders) : 0;

    const dryFoS = calculateSlopeFoS(slope, false);
    const wetFoS = calculateSlopeFoS(slope, true);
    ...
    const exports = useMemo(() => {
        const fFuelPrice = parseFloat(capexOpex.fuelPrice) || 0;
        const fProdVolume = parseFloat(capexOpex.prodVolume) || 0;
        const sAngle = parseFloat(slope.angle) || 0;
        const sFrictionAngle = parseFloat(slope.frictionAngle) || 0;
        const sUnitWeight = parseFloat(slope.unitWeight) || 0;
        const sWaterTableDepth = parseFloat(slope.waterTableDepth) || 0;

        const surpac = `X,Y,Z,Density,Copper_Grade_Pct,Gold_Grade_Gpt,Rock_Type
0,0,0,${fFuelPrice > 0 ? '2.65' : '2.70'},1.45,2.15,Ore
...
; Amortized unit cost: $${(fProdVolume > 0 ? 12.5 : 0).toFixed(2)}/t
; Slope FoS (Dry): ${dryFoS.toFixed(3)}
...
block cut joint-set angle ${sAngle} spacing 10 origin 50 50 -10
block cut joint-set angle ${(sFrictionAngle * 1.1).toFixed(0)} spacing 15 origin 50 50 -25

; Geotechnical Rockmass Material definitions
material 1 name "Siltstone_Highwall"
  density ${Math.floor(sUnitWeight * 100)}
  bulk 4.2e9
  shear 2.5e9
...
pore grid water-table depth ${sWaterTableDepth}
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 shadow-lg shadow-slate-950/50">
                        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4 mb-6">
                            ...
                            <button
                                ...
                                className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold border border-amber-600 rounded px-4 py-2 text-xs transition shadow flex items-center gap-1.5 cursor-pointer"
                            >
                                🖨️ Print Report
                            </button>
                        </div>

                        <div className="printable-audit-sheet bg-white text-slate-900 p-8 rounded-lg shadow-inner max-h-[450px] overflow-y-auto space-y-6 text-sm font-serif">
                            ...
                            <div className="space-y-3 font-sans text-xs">
                                ...
                                <p className="leading-relaxed">
                                    ...
                                    ${sellingPrice}/t. Mining blocks located beyond this ratio boundary are deemed economically infeasible and must be marked as waste dump allocations.
                                </p>
                            ...
                            <div className="space-y-3 font-sans text-xs">
                                ...
                                <p className="leading-relaxed">
                                    Logistical simulations sizing {fNumTrucks} haul trucks with {fNumLoaders} shovel loaders calculated a dynamic match factor of <strong>{fleetMatchFactor.toFixed(3)}</strong>.
                            ...
                            <div className="space-y-3 font-sans text-xs">
                                ...
                                <p className="leading-relaxed">
                                    Slope stability models using Bishop's Simplified method evaluated factors of safety of <strong>{dryFoS.toFixed(3)} (Dry)</strong> and <strong>{wetFoS.toFixed(3)} (Saturated)</strong>.
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 shadow-lg shadow-slate-950/50">
                        ...
                        <div className="flex border-b border-slate-800 gap-2 mb-4">
                            <button
                                ...
                                className={`px-3 py-1.5 font-mono text-xs transition border-b-2 font-bold ${currentSelectedFormat === 'surpac' ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 font-mono text-xs transition border-b-2 font-bold ${currentSelectedFormat === 'dec3' ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 font-mono text-xs transition border-b-2 font-bold ${currentSelectedFormat === 'arcgis' ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                                    }`}
                            >
                        ...
                        <div className="p-2 space-y-1">
                            {filteredSections.map(sec => (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2 rounded-lg text-xs font-semibold transition ${activeManualTab === sec.id
